            ...
            RoleManager<ApplicationRole> roleManager)
        {
            ...
            if (!await roleManager.RoleExistsAsync(Roles.Moderator.ToString()))
            {
                await roleManager.CreateAsync(new ApplicationRole(Roles.Moderator.ToString())
                    { Id = "78ece2ec-9b7f-4fd5-b2c5-c2ae7ab036ed" });
            }
            ...
            if (!await roleManager.RoleExistsAsync(Roles.Basic.ToString()))
            {
                await roleManager.CreateAsync(new ApplicationRole(Roles.Basic.ToString())
                    { Id = "dcc064f6-1dc9-40f1-ab54-9ce262ad1f22" });
            }
        }
    }
}